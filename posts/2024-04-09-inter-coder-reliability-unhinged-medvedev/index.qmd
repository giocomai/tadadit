...
description: "This post offers a preview of some results of early testing with using locally-deployed LLMs as coders"
...
- processing of Telegram posts of various length takes about one hour per 100 posts with a reasonably performing model such as Mistral-7b on my own laptop (without going into details, nvidia graphic card, and relatively recent i7 intel processor)
...
- these preliminary results appear to be rather encouraging, suggesting there may be valid use cases for this approach; more testing with more carefully designed prompts, perhaps replicating previous research and after more thorough exploration of the relevant literature, should be attempted.
...
## ![](giocomai_TEXT_AS_DATA__DATA_IN_THE_TEXT_pastel_colours_comics_s_ef58faff-dd90-41b5-a4dc-ff4f5e25aede.webp) Context {#top-left-img}
...
If it does, this could not only reduce the countless hours of repetitive, unrewarding and often unpaid work dedicated to this kind of activities in research, but could also add more flexibility, transparency and reproducibility to this strand of research. Reproducibility, for example, is often possibile in theory (when both codebook and original contents are made publicly available), but the disproportionate time ncessary for re-analysis makes this highly unlikely in practice.
...
As none of this is being conducted with enough attention to technicalities to be considered a serious endeavour, I decided to settle on the first issue that comes to mind when going through Medevedev's posts online, i.e. how incredibly unhinged these posts have become.
...
timing_df <- ratings_df |> 
  ...
  # dplyr::mutate(instruction = stringr::str_replace_all(instruction, pattern = instruction_v[1], replacement = names(instruction_v[1]))) |> 
 timing_df |>  
...
- are models subscettible to changes in the way the instruction is given?
...
It appears that when combining raters with different phrasing of the query, but grouping by model, it appear there is a somewhat acceptable degree of agreement only for Mistral 7b:
