...
**Earliest post with audio file**: `r min(transcribed_ru_df$datetime) |> lubridate::as_date()`

**Latest post with audio file**: `r max(transcribed_ru_df$datetime) |> lubridate::as_date()`
...
messages_with_prigozhin_id <- messages_df |> 
  dplyr::mutate(prigozhin_id = dplyr::if_else(
    condition = stringr::str_starts(string = text, pattern = "#"),
    true = stringr::str_extract(string = text, pattern = "#[[:digit:]]+"),
    false = NA_character_,
    missing = NA_character_
  )) |> 
  dplyr::select(id, prigozhin_id)

transcribed_ru_df <- read_transcriptions(path = "telegram_concordgroup_official_audio_transcribed_large")  |> 
  dplyr::left_join(messages_with_prigozhin_id, by = "id") |> 
  dplyr::relocate(id, prigozhin_id)
