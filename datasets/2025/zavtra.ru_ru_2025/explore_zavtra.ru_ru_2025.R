...
corpus_name <- "zavtra.ru_ru_2025"
corpus_file <- paste0(corpus_name, ".csv.gz", collapse = "")

## Step 1: Download the corpus ####

# download `zavtra.ru_ru_2025.csv.gz` from the main repository,
# or rely on the following to download the latest version from GitHub

library("piggyback")
pb_download(repo = "giocomai/tadadit", tag = corpus_name, file = corpus_file)
...
castarter::cas_explorer(
  corpus = arrow::open_dataset("zavtra.ru_ru_2025"),
  default_pattern = "коллективн[а-я]+ Запад",
  title = corpus_name
)
...
## Count matches in R
cas_count(corpus = corpus_df, pattern = "коллективн[а-я]+ Запад")
