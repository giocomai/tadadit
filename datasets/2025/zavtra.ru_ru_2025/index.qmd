---
title: "zavtra.ru_ru_2025"
description: "Corpus based on the website of Russian weekly newspaper 'Zavtra' (in Russian, 1996-2024)"
...
source(fs::path("..", "russian_institutions_2025", "release_functions_2025.R"))
source(fs::path("..", "russian_institutions_2025", "release_setup_2025.R"))
...
Official release with DOI now available on *Discuss Data* (2025 version to be released soon):
...
```{r corpus_2024}
corpus_2024_file <- fs::path(base_folder, "zavtra.ru_ru_2024.csv.gz")

if (!fs::file_exists(corpus_2024_file)) {
  download.file(url = "https://discuss-data.net/dataset/dae0b5b8-157f-471a-bb3c-ea24e0b2a7f7/files/f2f748ea-bd11-4cd2-b4b6-f57a7ac1b5b0/",
                destfile = corpus_2024_file)  
}

corpus_2024_df <- readr::read_csv(corpus_2024_file)

corpus_2025_df <- corpus_df
```




```{r corpus_id_combo_df}
corpus_id_combo_df <- corpus_2024_df |> 
  dplyr::select(id, url) |> 
  dplyr::rename(id_2024 = id) |> 
  dplyr::left_join(corpus_2025_df |> 
  dplyr::select(id, url) |> 
    dplyr::rename(id_2025 = id),
  by = "url") |> 
  dplyr::mutate(id_check = id_2024==id_2025) 

# corpus_id_combo_df |> dplyr::filter(!id_check)
# 
# corpus_id_combo_df |> dplyr::filter(is.na(id_2025))

corpus_id_combo_back_df <- corpus_2025_df |> 
  dplyr::filter(date < as.Date("2024-01-01")) |> 
  dplyr::select(id, url) |> 
    dplyr::rename(id_2025 = id) |> 
  dplyr::left_join(corpus_2024_df |> 
  dplyr::select(id, url) |> 
  dplyr::rename(id_2024 = id),
  by = "url") |> 
  dplyr::mutate(id_check = id_2024 == id_2025)
...
```{r corpus_version_combo_df, echo=FALSE}
corpus_version_combo_df <- corpus_2025_df |> 
  dplyr::rename(id_2025 = id, 
                doc_id_2025 = doc_id) |> 
  dplyr::left_join(y = corpus_2024_df |> 
                     dplyr::select(doc_id, id, url),
                   by = "url")

max_previous_id <- max(corpus_2024_df[["id"]])
n_na <- sum(is.na(corpus_version_combo_df[["id"]]))


corpus_version_combo_df[["id"]][is.na(corpus_version_combo_df[["id"]])] <- sum(max_previous_id, 1):sum(max_previous_id, n_na)

# check
testthat::test_that(desc = "All items have an id",
                    code = testthat::expect_equal(
                      object = sum(is.na(corpus_version_combo_df[["id"]])),
                      expected = 0)
)

corpus_version_combo_df <- corpus_version_combo_df |> 
  dplyr::mutate(doc_id = paste0("zavtra.ru_ru_", id)) |> 
  dplyr::relocate(doc_id, .before = doc_id_2025) |> 
  dplyr::relocate(id, .before = id_2025) |> 
  dplyr::select(-doc_id_2025, -id_2025) 


  # dplyr::mutate(id = dplyr::if_else(condition = is.na(id), true = id_2025, false = id),
  #               doc_id = dplyr::if_else(condition = is.na(doc_id), true = doc_id_2025, false = doc_id))

corpus_df <- corpus_version_combo_df
```
